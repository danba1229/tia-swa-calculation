...
function buildApiUrl(criteria) {
  ...
  if (!baseUrl || !operationPath || !serviceKey) {
    const missing = [
      ...
      !baseUrl ? "TIA_API_BASE_URL" : "",
      !operationPath ? "TIA_API_OPERATION_PATH" : "",
    ].filter(Boolean).join(", ");
...
export async function fetchTiaProjects(criteria = {}) {
  ...
  if (text.trim().startsWith("<")) {
    throw new Error("교통영향평가 API 응답 형식 오류: JSON 응답 경로와 파라미터를 lib/tiaApi.js에서 확인해 주세요.");
  }
  ...
  return {
    ...
    projects: filtered,
    payload,
    requestUrl: url.toString().replace(readFirstEnv(["DATA_GO_KR_SERVICE_KEY", "TIA_DATAGOKR"]), "********"),
  };
}
