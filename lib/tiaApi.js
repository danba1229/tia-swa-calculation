...
function normalizeProject(raw, index, source) {
  ...
  const location = pick(raw, ["위치", "사업위치", "사업지위치", "위치상세", "소재지", "대상지", "주소", "location", "addr", "address", "adres"]);
  const projectType = pick(raw, ["사업구분", "사업구분코드명", "사업유형", "사업유형코드명", "사업종류", "projectType", "bizType", "구분"]) || "미분류";

  return {
    id: pick(raw, ["사업아이디", "id", "ID", "관리번호", "사업번호", "접수번호", "seq", "sn"]) || `${source}-${index + 1}`,
    ...
    siteArea: pick(raw, ["대지면적", "부지면적", "사업지면적", "사업면적", "시설계획면적", "siteArea", "area"]),
    ...
    reviewResult: pick(raw, ["심의내용결과", "심의결과", "검토결과", "reviewResult", "결과", "추진단계", "진행상태", "status"]),
    registeredDate: pick(raw, ["사업등록일", "등록일", "접수일", "고시일", "심의일", "조사일", "registeredDate", "regDate"]),
    source,
    raw,
  };
}
...
async function fetchEndpoint(endpoint, criteria, serviceKey) {
  ...
  const locationKeyword = String(criteria.sigungu || criteria.sido || "").trim();
  const urls = primaryUrl.hostname === "api.odcloud.kr" && locationKeyword
    ? ["위치", "사업위치", "위치상세"].map((fieldName) => {
  ...
  for (const url of urls) {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`${endpoint.label} 호출 실패(${response.status})${hint}${preview}`);
      }
...
export async function fetchTiaProjects(criteria = {}) {
  ...
  const filtered = merged.filter((project) => (
    matchesAdminName(project, criteria.sido)
    && matchesAdminName(project, criteria.sigungu)
