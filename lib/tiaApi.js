...
function readTotalCount(payload) {
  const candidates = [
...
async function fetchEndpoint(endpoint, criteria, serviceKey) {
  ...
  if (isTiaSystem) {
    ...
    for (let pageNo = 1; pageNo <= maxPages; pageNo += 1) {
      ...
      try {
        const payload = await requestJson(pageUrl, endpoint.label);
        payloads.push(payload);
        const { pageItems, addedCount } = addPayloadItems(payload, pageNo);
        const totalCount = readTotalCount(payload);

        repeatedPages = addedCount === 0 ? repeatedPages + 1 : 0;
        if (!pageItems.length) break;
        if (Number.isFinite(totalCount) && pageNo * pageSize >= totalCount) break;
        if (pageItems.length < pageSize) break;
        if (repeatedPages >= maxRepeatedPages) break;
      } catch (error) {
...
function matchesAdminCriteria(project, sido, sigungu) {
